...
export async function POST(request: Request) {
  ...
  // 1b. Check for duplicate participants (same name + birth_date already registered for this event)
  {
    ...
    // Query existing participants in active registrations for this event
  ...
  // Manual payment discount per person
  ...
  if (!regGroup.is_default) {
    ...
    if (defaultGroup) {
      ...
      defIsEarlyBird =
        ...
        defNow < new Date(defEffectiveDeadline) &&
        (event?.early_registration_start == null || defNow >= new Date(event.early_registration_start));
  ...
  const fundingSources = await loadFundingForGroup(admin, registrationGroupId, nightsCount);
  ...
  syncRegistration(eventId, registration.id).catch((err) =>
    logger.error("[registration/submit] Google Sheets sync failed", { error: String(err) })
  );
