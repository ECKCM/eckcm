...
import { DailyMealReportClient } from "./daily-meal-report-client";
...
export default async function DailyMealReportPage() {
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-4 py-8 sm:py-10">
      <header className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold sm:text-3xl">Daily Meal Report</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">
            Served meals by age tier for a single day.
          </p>
        </div>
      </header>

      <DailyMealReportClient
        events={(events ?? []).map((e) => ({
          id: e.id,
          name_en: e.name_en,
          year: e.year,
