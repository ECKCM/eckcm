...
export async function POST(request: Request) {
  ...
  if (pendingCardPayments && pendingCardPayments.length > 0) {
    try {
      ...
      for (const payment of pendingCardPayments) {
        if (payment.stripe_payment_intent_id) {
          try {
            ...
            logger.warn("[payment/check-submit] Failed to cancel Stripe PI", {
      ...
      logger.info("[payment/check-submit] Cleaned up orphaned Stripe payments", {
        count: pendingCardPayments.length,
      });
    } catch (err) {
      logger.warn("[payment/check-submit] Error cleaning up Stripe payments", {
        error: String(err),
      });
    }
  }
  ...
  if (paymentInsertError) {
    logger.error("[payment/check-submit] Failed to create payment record", {
  ...
  if (registrationUpdateError) {
    logger.error("[payment/check-submit] Failed to update registration", {
  ...
  if (memberships && memberships.length > 0) {
    ...
    if (newTokens.length > 0) {
      const { error: insertError } = await admin
        ...
        .insert(newTokens);
      if (insertError) {
        logger.error("[payment/check-submit] Failed to insert epass tokens", { error: String(insertError) });
      } else {
        tokensGenerated = newTokens.length;
      }
    }
  }
  logger.info("[payment/check-submit] Inactive E-Pass tokens generated", { tokensGenerated });

  // Send email with Check payment instructions (non-blocking)
  after(async () => {
    try {
      await sendConfirmationEmail(registrationId, null, { paymentMethod: "CHECK" });
    } catch (err) {
      logger.error("[payment/check-submit] Failed to send Check instructions email", { error: String(err) });
    }
  });
  ...
  syncRegistration(registration.event_id, registrationId).catch((err) =>
    logger.error("[payment/check-submit] Google Sheets sync failed", { error: String(err) })
  );
  ...
  return NextResponse.json({ success: true });
    ...
    logger.error("[payment/check-submit] Unhandled error", { error: String(err) });
    return NextResponse.json(
