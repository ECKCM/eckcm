...
export async function POST(request: Request) {
  ...
  // Reuse the same schema (just needs registrationId)
  ...
  // On-site cash/check avoids card processing fees, so it receives the same
  // manual-payment discount as Zelle/Check.
  let discountCents = 0;
  {
  ...
  if (discountCents > 0) {
    ...
    if (!existingDiscount) {
      await admin.from("eckcm_invoice_line_items").insert({
        ...
        description_en: "Manual Payment Discount (On-Site)",
        description_ko: "현장 결제 할인",
        quantity: 1,
  ...
  if (pendingCardPayments && pendingCardPayments.length > 0) {
    try {
      ...
      for (const payment of pendingCardPayments) {
        if (payment.stripe_payment_intent_id) {
          try {
            ...
            logger.warn("[payment/onsite-submit] Failed to cancel Stripe PI", {
      ...
      logger.info("[payment/onsite-submit] Cleaned up orphaned Stripe payments", {
        count: pendingCardPayments.length,
      });
    } catch (err) {
      logger.warn("[payment/onsite-submit] Error cleaning up Stripe payments", {
        error: String(err),
      });
    }
  }
  ...
  if (paymentInsertError) {
    logger.error("[payment/onsite-submit] Failed to create payment record", {
  ...
  if (registrationUpdateError) {
    logger.error("[payment/onsite-submit] Failed to update registration", {
  ...
  // Generate E-Pass tokens with is_active = false (activated when admin confirms payment on-site)
  ...
  if (memberships && memberships.length > 0) {
    ...
    if (newTokens.length > 0) {
      const { error: insertError } = await admin
        ...
        .insert(newTokens);
      if (insertError) {
        logger.error("[payment/onsite-submit] Failed to insert epass tokens", { error: String(insertError) });
      } else {
        tokensGenerated = newTokens.length;
      }
    }
  }
  logger.info("[payment/onsite-submit] Inactive E-Pass tokens generated", { tokensGenerated });

  // Send email confirming the registration was submitted (non-blocking)
  after(async () => {
    try {
      await sendConfirmationEmail(registrationId, null, { paymentMethod: "ONSITE" });
    } catch (err) {
      logger.error("[payment/onsite-submit] Failed to send confirmation email", { error: String(err) });
    }
  });
  ...
  syncRegistration(registration.event_id, registrationId).catch((err) =>
    logger.error("[payment/onsite-submit] Google Sheets sync failed", { error: String(err) })
  );

  // Audit log
  await admin.from("eckcm_audit_logs").insert({
    user_id: user.id,
    action: "ONSITE_PAYMENT_SUBMITTED",
  ...
  return NextResponse.json({ success: true });
    ...
    logger.error("[payment/onsite-submit] Unhandled error", { error: String(err) });
    return NextResponse.json(
