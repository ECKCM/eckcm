  ...
  if (!applyGeneralFeesToMembers || !applyMealFeesToMembers) {
    ...
    if (defaultGroup) {
      ...
      if (!applyGeneralFeesToMembers) {
        const defRegFee = defaultLinkedFees.find((f: any) => f.code === "REG_FEE");
        const defEarlyBird = defaultLinkedFees.find((f: any) => f.code === "EARLY_BIRD");
        defaultRegistrationFeePerPerson =
          defaultGroup.global_registration_fee_cents ?? defRegFee?.amount_cents ?? 0;
        defaultEarlyBirdFeePerPerson =
          defaultGroup.global_early_bird_fee_cents ?? defEarlyBird?.amount_cents ?? null;
        defaultIsEarlyBird =
